...
export function StoreDay() {
  ...
  /** Задача дня ссылается на шаблон, который и открываем для редактирования. */
    ...
    <AsyncView state={state}>
      {(data) => {
        const completed = data.tasks.filter(
          (task) => task.status === 'done_on_time' || task.status === 'done_late',
        ).length;
        const problems = data.tasks.filter(
          (task) => task.status === 'overdue' || task.status === 'unclaimed' || task.status === 'missed',
        );
