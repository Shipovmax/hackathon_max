...
4. В начале смены бот присылает доску смены: список задач и кнопка у каждой. Перед сроком задачи приходят два напоминания, после нажатия бот просит фото именно для названной задачи, а затем отдаёт обновлённый список — его же получают остальные сотрудники смены.
...
| `REMINDER_FIRST_MINUTES_BEFORE`, `REMINDER_FINAL_MINUTES_BEFORE`, `CLAIM_ESCALATION_MINUTES_BEFORE`, `PHOTO_WAIT_MINUTES`, `SHIFT_BOUNDARY_TOLERANCE_MINUTES` | Параметры логики задач и смен. Напоминания отсчитываются от срока задачи: плановое время плюс её допуск |
